...
public class KMeansMatrices {
    ...
    public static ArrayList<RealMatrix> freqMatrix(ArrayList<RealMatrix> input) {
        ...
        for (int i = 0; i < itNum; i++) {
            ...
            if(i < itNum - 1) {
                while (clusters.size() < k) {
                    if (clusters.get(ind % maxInd).matrices.size() > 1) {
        ...
        for (int i = 0; i < clusters.size(); i++) {
            System.out.println("Cluster (" + (i + 1) + ") size " + clusters.get(i).matrices.size());
        }
...
class Cluster implements Comparable {
    ...
    public int compareTo(Object o) {
        return Double.compare(((Cluster) o).dist(), this.dist());
    }
